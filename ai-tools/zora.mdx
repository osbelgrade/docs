...
Zora is named after the Serbian "зора" (dawn/daybreak) and she functions as an intelligent query interface for users attempting to understand COS:B protocols documented throughout this site
...
Zora processes natural language queries about COS:B documentation using agentic retrieval-augmented generation powered by Claude Sonnet 4. When users submit protocol clarification requests, Zora:
...
<AccordionGroup>
  ...
  <Accordion title="Provides Source Citations" icon="message-quote" iconType="solid">
    Links directly to referenced pages within COS:B modules, enabling users to examine full documentation context.
  </Accordion>
...
Append `?assistant=open` to any COS:B page URL to launch Zora on page load
...
<AccordionGroup>
  <Accordion title="Scope Boundaries" icon="circle-stop" iconType="solid">
    Zora operates only on documented COS:B content. Questions about protocols not yet observed or documented will receive acknowledgment of documentation gaps rather than speculation.
  </Accordion>
  ...
  <Accordion title="External Resources" icon="circle-stop" iconType="solid">
    Zora cannot access external Belgrade documentation or general travel resources. Responses derive exclusively from COS:B systematic observations.
  </Accordion>
</AccordionGroup>

## Query Analytics

COS:B monitors query patterns to identify:
...
- **Contextual Retrieval**: RAG architecture accessing full COS:B documentation corpus
- **Source Attribution**: Automatic citation linking to referenced observations
- **Code Generation**: Python snippet creation following COS:B template standards
...
## Integration with COS:B Methodology
